...
class GraphTraversalServiceImpl(dao: GraphDAO) extends GraphTraversalService {
  ...
  def findShortestPath(
    ...
    val trimmedVertices =
      dao.listLocations.filterNot(n => (n == originUnLocode) || (n == destinationUnLocode))
    ...
    for (i <- 0 to candidateCount) {
      ...
      var transitEdges = List(
        new TransitEdge(firstVoyageNumber, originUnLocode, firstLegTo, fromDate, toDate)
      )
      ...
      val transitEdge =
        new TransitEdge(voyageNumber, lastLegFrom, destinationUnLocode, fromDate, toDate)
      transitEdges = transitEdges ::: List(transitEdge)
  ...
  private def nextDate(date: Date): Date = {
    ...
    new Date(
      date.getTime() + GraphTraversalServiceImpl.ONE_DAY_MS + (random.nextInt(
        1000
      ) - 500) * GraphTraversalServiceImpl.ONE_MIN_MS
    )
  }
