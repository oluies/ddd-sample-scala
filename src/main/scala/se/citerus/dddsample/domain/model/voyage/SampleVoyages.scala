...
object SampleVoyages extends DateTestUtil {
  ...
  private def createVoyage(id: String, from: Location, to: Location): Voyage =
    new Voyage(
      new VoyageNumber(id),
      new Schedule(List(new CarrierMovement(from, to, new Date(), new Date())))
    )
  ...
  private val voyageList: List[Voyage] = List(
    v100,
    v200,
    v300,
    v400,
  ...
  private val voyageMap: Map[VoyageNumber, Voyage] = Map() ++ voyageList.map { v =>
    (v.voyageNumber, v)
  }
