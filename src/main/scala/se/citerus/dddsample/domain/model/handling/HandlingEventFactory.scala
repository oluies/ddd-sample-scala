...
final class HandlingEventFactory(
  ...
  def createHandlingEvent(
    ...
    val cargo    = cargoRepository.find(trackingId).getOrElse(throw new UnknownCargoException(trackingId))
    val location = locationRepository.find(unlocode).getOrElse(throw new UnknownLocationException(unlocode))
    ...
    try
      voyage match
        case Some(v) => HandlingEvent(cargo, completionTime, registrationTime, eventType, location, v)
