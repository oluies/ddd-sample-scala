...
final class CargoFactory(
  ...
  def createCargo(originUnLoCode: UnLocode, destinationUnLoCode: UnLocode, arrivalDeadline: Instant): Cargo =
    val trackingId  = cargoRepository.nextTrackingId()
    val origin      = locationRepository.find(originUnLoCode).getOrElse(
      throw new NoSuchElementException(s"Unknown origin UN locode: ${originUnLoCode.idString}")
    )
    val destination = locationRepository.find(destinationUnLoCode).getOrElse(
      throw new NoSuchElementException(s"Unknown destination UN locode: ${destinationUnLoCode.idString}")
    )
