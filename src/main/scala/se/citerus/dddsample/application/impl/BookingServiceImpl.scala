...
class BookingServiceImpl(
  ...
  def changeDestination(trackingId: TrackingId, unLocode: UnLocode): Unit = {
    val cargo = cargoRepository.find(trackingId).getOrElse {
      throw new IllegalArgumentException("Can't find cargo with tracking id = " + trackingId)
    }
    ...
    cargoRepository.store(cargo);
    logger.info(
      "Changed destination for cargo " + trackingId + " to " + routeSpecification.destination
    );
  }
