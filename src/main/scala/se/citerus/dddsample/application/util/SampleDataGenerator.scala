...
class SampleDataGenerator extends ServletContextListener {
  ...
  private val base = new Timestamp(
    new SimpleDateFormat("yyyy-MM-dd").parse("2008-01-01").getTime() - 1000L * 60 * 60 * 24 * 100
  )
  ...
  override def contextInitialized(event: ServletContextEvent): Unit = {
    ...
    loadHibernateData(
      ...
      getBean(context, classOf[HandlingEventRepository])
    );
  }
  ...
  def loadHibernateData(
    ...
    val _ = tt.execute(new TransactionCallbackWithoutResult() {
  ...
  def loadSampleData(jdbcTemplate: JdbcTemplate, transactionTemplate: TransactionTemplate): Unit = {
    val _ = transactionTemplate.execute(new TransactionCallbackWithoutResult() {
